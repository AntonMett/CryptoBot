...
class private_api:
    ...
    def get_rig_stat(self):
        return self.request('GET', '/main/api/v2/mining/rigs2/', '', None)

    def delete_all_orders(self):
        return self.request('DELETE', '/exchange/api/v2/info/cancelAllOrders', '', None)

    def post_order(self, market, side, type, price, quanity):
        query = "minQuantity=&market={}&side={}&type={}&price={}&quantity={}".format(
            market, side, type, price, quanity)
    ...
    def create_hashpower_order(self, market, type, algorithm, price, limit, amount, pool_id, algo_response):
        ...
        algo_setting = self.algo_settings_from_response(
    ...
    def get_my_exchange_orders2(self, market, orderStatus):
        query = "market={}&orderStatus={}".format(
            market, orderStatus)
    ...
    def create_exchange_buy_market_order(self, market, quantity):
        query = "market={}&side=buy&type=market&secQuantity={}".format(
            market, quantity)
        return self.request('POST', '/exchange/api/v2/order', query, None)

    def create_exchange_sell_market_order(self, market, quantity):
        query = "market={}&side=sell&type=market&quantity={}".format(
            market, quantity)
        return self.request('POST', '/exchange/api/v2/order', query, None)
...
if __name__ == "__main__":
    ...
    parser.add_option('-b', '--base_url', dest="base",
                      help="Api base url", default="https://api2.nicehash.com")
    parser.add_option('-o', '--organization_id',
                      dest="org", help="Organization id")
    parser.add_option('-k', '--key', dest="key", help="Api key")
    parser.add_option('-s', '--secret', dest="secret",
                      help="Secret for api key")
    parser.add_option('-m', '--method', dest="method",
                      help="Method for request", default="GET")
    parser.add_option('-p', '--path', dest="path",
                      help="Path for request", default="/")
    parser.add_option('-q', '--params', dest="params",
                      help="Parameters for request")
    parser.add_option('-d', '--body', dest="body", help="Body for request")

    options, args = parser.parse_args()

    private_api = private_api(
        options.base, options.org, options.key, options.secret)
    ...
    try:
        response = private_api.request(
            options.method, options.path, params, options.body)
