...
def available_currency(currency):
    return (private_api.get_accounts_for_currency(currency).get('available'))
...
def current_btcusdt_price():
    return (public_api.get_current_price().get('BTCUSDT'))
...
def make_new_buy_order(multiply, btcusdt=current_btcusdt_price()):
    price = multiply*btcusdt
    quantity = float(available_currency('USDT'))/price
...
if __name__ == "__main__":
    ...
    print(
        f'BTC  MINIMAL BUYING PRICE IS SET TO : {maximum_BTC_buy_price} USDT')
    run_bot()
